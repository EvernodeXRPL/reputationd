...
const COMMON_CONFIG_KEYS = ['network', 'governorAddress', 'rippledServer', 'fallbackRippledServers'];
    ...
    static readConfig(configPath, mbXrplConfigPath = null, readSecret = false) {
        ...
        if (mbXrplConfigPath && fs.existsSync(mbXrplConfigPath)) {
            ...
            for (const e of Object.entries(mbXrplConfig.xrpl)) {
                const isCommon = COMMON_CONFIG_KEYS.includes(e[0]);
                const key = isCommon ? e[0] : `host${e[0].charAt(0).toUpperCase()}${e[0].slice(1)}`;
                if (isCommon || !(key in config.xrpl))
                    config.xrpl[key] = e[1];
            }
        }
