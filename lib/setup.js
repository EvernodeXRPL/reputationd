...
class Setup {
    ...
    newConfig(address = "", secretPath = "", accountMode = evernode.EvernodeConstants.ReputationAccountMode.OneToOne) {
        const baseConfig = {
    ...
    async prepareReputationAccount() {
        ...
        try {
            ...
            await hostClient.prepareReputationAccount(acc.address, acc.secret, acc.accountMode, { retryOptions: { maxRetryAttempts: MAX_TX_RETRY_ATTEMPTS, feeUplift: Math.floor(acc.affordableExtraFee / MAX_TX_RETRY_ATTEMPTS) } });
