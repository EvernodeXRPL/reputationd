...
class Setup {
    ...
    newConfig(address = "", secretPath = "", reimburseFrequency = false) {
        const baseConfig = {
            ...
            reimburseFrequency: reimburseFrequency,
            contractInstance: {}
        };
    ...
    async prepareReputationAccount() {
        ...
        console.log('test>>> reputationCfg:', JSON.stringify(config));
    ...
    // Upgrades existing reputationd data to the new version.
    async upgrade() {
    ...
    async updateReimburseConfig(newFrequency = false) {
        ...
        cfg.reimburseFrequency = newFrequency;
    ...
    async getReimbursementStatus() {
        ...
        const cfg = this.#getConfig();
        // TODO:- check below logic according to default config value
        if(cfg.reimburseFrequency > 0){
            throw e;
        }
