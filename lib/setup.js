...
class Setup {
    ...
    newConfig(address = "", secretPath = "", reimburseFrequency = false) {
        const baseConfig = {
    ...
    async prepareReputationAccount(accountMode = evernode.EvernodeConstants.ReputationAccountMode.OneToOne) {
        ...
        try {
            ...
            await hostClient.prepareReputationAccount(acc.address, acc.secret, accountMode, { retryOptions: { maxRetryAttempts: MAX_TX_RETRY_ATTEMPTS, feeUplift: Math.floor(acc.affordableExtraFee / MAX_TX_RETRY_ATTEMPTS) } });
    ...
    checkConfigChanges() {
        ...
        if (cfg.version != appenv.REPUTATIOND_VERSION) {
    ...
    async repInfo() {
        ...
        try {
            ...
            console.log(`Evernode Reputation Hook address: ${config?.reputationAddress}`);
            ...
            console.log(`Host reputation-delegate account address: ${acc.address}`);
    ...
    async updateReimburseConfig(newFrequency = false) {

        // Do a simple frequency change in the config.
        const cfg = this.#getConfig();
        cfg.reimburse.frequency = newFrequency;
