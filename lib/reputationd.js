...
class ReputationD {
    ...
    async #cacheScores() {
        ...
        try {
            ...
            if (valid)
                this.cfg.scores = { moment: this.cfg.contractInstance?.created_moment, clusterSize: stat.currentUnl?.length, scores: res?.scores };
        } catch (e) {
            console.error('Error occurred while reading the scores:', e);
            delete this.cfg.scores;
        }
        ...
        await this.#queueAction(async (submissionRefs) => {
            ...
            if (scheduledMoment == curMoment) {
                ...
                if (this.lastReputationMoment === 0 || curMoment !== this.lastReputationMoment) {
                    ...
                    try {
                        await this.hostClient.sendReputations(appenv.SCORE_VERSION, scoreRes?.clusterSize ?? 0, scoreRes?.scores, { submissionRef: submissionRefs?.refs[0], ...this.#prepareHostClientFunctionOptions() });
