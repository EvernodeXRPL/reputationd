...
class ReputationD {
    ...
    async #auditInstance() {
        // TODO: Implement peer/user port connectivity and ssl validity here.
        return true;
    }
    ...
    async #getScores() {
        ...
        try {
            ...
            return { clusterSize: stat.currentUnl?.length, scores: res?.scores };
        } catch (e) {
    ...
    #checkRequirements() {
        ...
        if (!hasIPv4) {
            console.log('IPv4 support is required to be a Evernode host and participate in reputation.');
            return false;
        }
        ...
        if (evernode.Defaults.values.minVersions?.reputationD) {
            ...
            if (major > minMajor)
                ...
                outdated = false;
            if (outdated) {
                console.log(`ReputationD version is outdate. Found: ${this.cfg.version}, Required: ${evernode.Defaults.values.minVersions?.reputationD}`);
                return false;
            }
        }
    ...
    async #sendReputations() {
        ...
        await this.#queueAction(async (submissionRefs) => {
            ...
            if (curMoment !== createdMoment || this.cfg.contractInstance.status !== ContractStatus.Running) {
            ...
            if (scheduledMoment == curMoment) {
                ...
                if (this.lastReputationMoment === 0 || curMoment !== this.lastReputationMoment) {
                    ...
                    if (curMoment === (createdMoment + 1))
                        scoreRes = await this.#getScores();

                    console.log(`Reporting reputations at Moment ${curMoment} ${scoreRes?.scores ? 'With scores' : 'Without scores'}...`);

                    try {
                        await this.hostClient.sendReputations(scoreRes?.clusterSize ?? 0, scoreRes?.scores, { submissionRef: submissionRefs?.refs[0], ...this.#prepareHostClientFunctionOptions() });
