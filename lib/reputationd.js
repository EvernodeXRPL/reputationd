...
class ReputationD {
    ...
    async init() {
        ...
        // Start reputation reimbursement job.
        this.#startReputationReimbursementScheduler();
    }
    ...
    async #processReimbursement() {
        const currentReimbursementCycle = this.cfg.reimburse.reimbursements.filter(item => item.reimburseTx === null);

        if (currentReimbursementCycle.length === 0) {
            console.log("No items to reimburse.");
            return;
        }

        if (currentReimbursementCycle.length > 0) {
            let reimburse_amount = currentReimbursementCycle.length * this.cfg.xrpl.hostLeaseAmount;
            let retry = false;
            try {
                const reimbursementTx = await this.hostClient.xrplAcc.makePayment(this.cfg.xrpl.address, reimburse_amount.toString(), 'EVR', this.hostClient.config.evrIssuerAddress, null, {
                    // maxLedgerIndex: this.hostClient.xrplApi.ledgerIndex + 1, //for testing only. remove this from final implementation.
                }).catch(() => {
                    retry = true;
                });;
                ...
                if (retry) {
                    const reimbusementTx = await this.hostClient.xrplAcc.makePayment(this.cfg.xrpl.address, reimburse_amount.toString(), 'EVR', this.hostClient.config.evrIssuerAddress);
                    if (!reimbusementTx) {
                        throw "Reputation account reimbursement failed";
                    }
                }

                // Update the reimbursement data with the transaction details
                this.cfg.reimburse.reimbursements.forEach(item => {
                    if (currentReimbursementCycle.some(cycleItem => cycleItem.contractInstance.name === item.contractInstance.name)) {
                        item.reimburseTx = reimbursementTx.id;
                    }
                });
                this.#persistConfig();
                ...
                console.error("Error in reimbursement:", error);
            }
        }
    }
    ...
    async #startReputationReimbursementScheduler() {
        const momentSize = this.hostClient.config.momentSize /200; //reduced moment size for testing.
        let timeout = momentSize * 1000;

        this.#readConfig();
        let reimburseFrequency = this.cfg.reimburse.frequency;
        if(!isNaN(reimburseFrequency)){
            timeout *= reimburseFrequency;
        }
        ...
        const scheduler = async () => {
            ...
            let reimburseFrequency = this.cfg.reimburse.frequency;
            if(!isNaN(reimburseFrequency)){
                await this.#processReimbursement();
                timeout *= reimburseFrequency;
            }
        ...
        await this.#queueAction(async (submissionRefs) => {
            ...
            if (curMoment > createdMoment) {
                ...
                if (retry) {
                    ...
                    this.#readConfig();
                    if (this.cfg.reimburse.frequency) {
                        this.cfg.reimburse.reimbursements.push({ 'contractInstance': this.cfg.contractInstance, 'reimburseTx': null });
                        this.#persistConfig();
                    }
                }
