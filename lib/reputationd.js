...
class ReputationD {
    ...
    async #sendReputations() {
        ...
        if (!eligible) {
            console.log(`Skipping reputation send due to ineligibility for reputation.`);
            return;
        }
        ...
        await this.#queueAction(async (submissionRefs) => {
            ...
            if (scheduledMoment == currentMoment) {
                ...
                if (this.lastReputationMoment === 0 || currentMoment !== this.lastReputationMoment) {
                    ...
                    console.log(`Reporting reputations at Moment ${currentMoment} ${scoreRes?.scores ? 'With scores' : 'Without scores'}...`);

                    try {
                        const buffer = await this.hostClient.prepareHostReputationScores(appenv.SCORE_VERSION, scoreRes?.clusterSize ?? 0, scoreRes?.scores);
                        await this.hostClient.sendReputations(buffer?.toString('hex')?.toUpperCase(), { submissionRef: submissionRefs?.refs[0], ...this.#prepareHostClientFunctionOptions() });
