...
class ReputationD {
    ...
    async #startReputationContractScheduler() {
        ...
        // If deploy widow has passed or, If we are to deploy now but not registered for next moment, Schedule for next moment.
    ...
    async #createReputationContract() {
        ...
        await this.#queueAction(async (submissionRefs) => {
            ...
            const momentStartTimestamp = await this.hostClient.getMomentStartIndex(curMoment);
            const momentSize = this.hostClient.config.momentSize;
